    ...
    while(1)
    {
        ...
        case MAIN_IDLE:
            ...
            if(States.But1)
            {
                ...
                States.Main = MAIN_LED1;        // change state to Led 1
                States.But1 = 0;                // reset button 1
        ...
        case MAIN_LED1:
            ...
            if(States.But1)
            {
                States.Main = MAIN_LED2;            // set sate to led2
                States.But1 = 0;
                reset_timeout();                    // reset time
            ...
            if(States.Time)
            {
                States.Main = MAIN_IDLE;
                States.Time = 0;
                stop_timeout();                     // stop timer for idle state
        ...
        case MAIN_LED2:
            ...
            if(States.But1)
            {
                States.Main = MAIN_LED1;            // change state back to led1
                States.But1 = 0;
                reset_timeout();                    // reset time
            ...
            if(States.Time)
            {
                States.Main = MAIN_IDLE;                // back to idle when time exceeds
                States.Time = 0;
                stop_timeout();                         // stop time for idle state
...
{
    /* TODO TIMEOUT START */
    SFRIE1 |= WDTIE;                    // start watchdog interrupt
    SFRIFG1 &= ~WDTIFG;                 // set flag to low

    WDTCTL = WDTPW |  WDTTMSEL | WDTSSEL_1 | WDTIS_4;           // choose ACLK = 10 kHz, divide by 2^19, 3.3 sec interval
...
void reset_timeout(void)                // Reset the timeout counter.
{
    ...
    WDTCTL = WDTPW | WDTCNTCL | WDTTMSEL | WDTSSEL_1 | WDTIS_4;     // clear timer
...
__interrupt void WDT_ISR(void)
{
    ...
    States.Time = 1;
    SFRIFG1 &= ~WDTIFG;                                             // clear watchdog interrupt flag
}
...
__interrupt void Timer0_A1_ISR (void)
{
    ...
    case TA0IV_TA0CCR1:                 // TA0 CCR1
        ...
        if(!(P1IN & BIT1))              // Validate button S2 state: pushed.
        {
            ...
            if ((States.Main != MAIN_IDLE))  // check the state of fsm, But2 not valid for idle state
            {
    ...
    case TA0IV_TA0CCR2:                 // TA0 CCR2
        ...
        if(!(P4IN & BIT5))              // Validate button S1 state: pushed.
        {
            ...
            // Valid for all states of main FSM:
