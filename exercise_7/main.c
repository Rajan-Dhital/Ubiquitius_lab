...
void main(void)
{
    ...
    P4SEL1 |= BIT2;             // configure P4.2/A10 for ADC function
    P4SEL0 |= BIT2;
    ...
    REFCTL0 |= REFON | REFVSEL_2;        // Select internal ref = 2.5V
    ...
    while(1)
    {
        ...
        case ADC_T_SLOW:                // Initialize ADC to sample slow.
            ...
            ADC12CTL0 = 0;
            ADC12CTL0 = ADC12SHT0_6 | ADC12ON;                   // sampling time and ADC on
            ADC12CTL1 = ADC12SHS_1 | ADC12SHP  | ADC12CONSEQ_2; // Use TA0.1 to trigger, and repeated-single-channel
            ADC12CTL2 = ADC12RES_0;                                                // 8 bit resoulution
            ADC12MCTL0 = ADC12VRSEL_1 | ADC12INCH_10;                               // ADC input select; Vref+ = VREF, Pin A10
            ...
            TA0CCR0 |= 12499;                    // duty cycle of 0.25s
            TA0CCTL1 = OUTMOD_3;                      // TACCR1 set/reset
            TA0CCR1 = 12499;                           // TACCR1 PWM Duty Cycle
        ...
        case ADC_T_FAST:                // Initialize ADC to sample fast.
            ...
            ADC12CTL0 = 0;
            ADC12CTL0 = ADC12SHT0_6 | ADC12ON | ADC12MSC;            // sampling time and ADC on, multiple sample conversion on rising edge
            ADC12CTL1 = ADC12SHP  | ADC12SSEL_2 | ADC12CONSEQ_2;        // self trigger, and repeated-single-channel
