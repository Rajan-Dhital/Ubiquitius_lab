...
*******************************************************************************/
    ...
    while(1)
    {
        /* Blink LEDs alternating with a period time of 1 s. */

        /* LED1 red on for one eighth period. */
        P4OUT |= BIT6;                  // Turn LED1 on.
        P1OUT &= ~BIT0;                 // Turn LED2 off.
        __delay_cycles(125000);         // Delay one eighth period 0.125 s.

        /* LED1 red off for one eighth period. */
        ...
        /* LED2 green on for one eighth period. */
        P4OUT &= ~BIT6;                 // Turn LED1 off.
        P1OUT |= BIT0;                  // Turn LED2 on.
        __delay_cycles(125000);         // Delay half period 0.5 s.

        /* LED2 green off for one eighth period. */
        P4OUT &= ~BIT6;                  // Turn LED1 off.
        P1OUT &= ~BIT0;                 // Turn LED2 off.
        __delay_cycles(125000);         // Delay one eighth period 0.125 s.
...
*******************************************************************************/
    ...
    while(1)
    {
        ...
        /* LED1 red on for one fourth period. */
        P4OUT |= BIT6;                  // Turn LED1 on.
        P1OUT &= ~BIT0;                 // Turn LED2 off.
        __delay_cycles(250000);         // Delay half period 0.25 s.


        /* LED1 red off for one fourth period. */
        P4OUT &= ~BIT6;                  // Turn LED1 off.
        P1OUT &= ~BIT0;                 // Turn LED2 off.
        __delay_cycles(250000);         // Delay half period 0.25 s.
        ...
        /* LED2 green on for one fourth period. */
        P4OUT &= ~BIT6;                 // Turn LED1 off.
        P1OUT |= BIT0;                  // Turn LED2 on.
        __delay_cycles(250000);         // Delay half period 0.25 s.

        /* LED1 green off for one fourth period. */
        P4OUT &= ~BIT6;                  // Turn LED1 off.
        P1OUT &= ~BIT0;                 // Turn LED2 off.
        __delay_cycles(250000);         // Delay half period 0.25 s.
